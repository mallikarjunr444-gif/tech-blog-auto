...
# Blog: technewsai.me | Author: Mallikarjun R
# AI: Groq Llama 3.3 70B + Grok | Posting: Blogger API v3
...
def get_access_token():
    ...
    sa_file = "beaming-oarlock-479403-b5-598523c82f8c.json"
    if os.path.exists(sa_file):
        try:
            ...
            print("  Service account error: " + str(e))

    # Try refresh token from environment
    refresh_token = os.environ.get("BLOGGER_REFRESH_TOKEN", "")
    client_id     = os.environ.get("BLOGGER_CLIENT_ID", "754470103610-o1vllj1gbv0uq3o6vl7h2brtpd5usvif.apps.googleusercontent.com")
...
def post_to_blogger_api(title, html, labels):
    # Post to Blogger API
    ...
    if resp.status_code in (200, 201):
        data = resp.json()
        post_url = data.get("url", BLOG_URL)
        print("  Posted: " + post_url)
        return post_url
    else:
        print("  API Error: " + str(resp.status_code) + " - " + resp.text[:200])
        raise Exception("Blogger API error: " + str(resp.status_code))


def post_article(title, html, labels):
    print("  Posting: " + title[:65] + "...")
    return post_to_blogger_api(title, html, labels)
...
def main():
    print("=" * 58)
    print("  TECH NEWS WITH AI - AUTO BLOG v5.0")
    print("  " + BLOG_URL)
    print("  Blogger API | AdSense-Ready | EEAT | SEO")
    print("=" * 58)
    success = 0
